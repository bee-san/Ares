...
use crate::filtration_system::{get_decoder_tagged_decoders, get_non_decoder_tagged_decoders, MyResults};
...
pub fn astar(input: String, result_sender: Sender<Option<DecoderResult>>, stop: Arc<AtomicBool>) {
    ...
    while !open_set.is_empty() && !stop.load(std::sync::atomic::Ordering::Relaxed) {
        trace!("Current depth is {:?}, open set size: {}", curr_depth, open_set.len());
        
        // Get the node with the lowest f value
        let current_node = open_set.pop().unwrap();
        let current_string = current_node.state;
        
        trace!("Processing node with cost {}, heuristic {}, total cost {}", 
               current_node.cost, current_node.heuristic, current_node.total_cost);
        ...
        if !decoder_tagged_decoders.components.is_empty() {
            trace!("Found {} decoder-tagged decoders to execute immediately", 
            ...
            match decoder_results {
                ...
                MyResults::Continue(results_vec) => {
                    ...
                    trace!("Processing {} results from decoder-tagged decoders", results_vec.len());
        ...
        if !non_decoder_decoders.components.is_empty() {
            trace!("Processing {} non-decoder-tagged decoders", 
            ...
            match decoder_results {
                ...
                MyResults::Continue(results_vec) => {
                    ...
                    trace!("Processing {} results from non-decoder-tagged decoders", results_vec.len());
